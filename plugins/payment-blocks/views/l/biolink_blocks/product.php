...
<div id="<?= 'biolink_block_id_' . $data->link->biolink_block_id ?>" data-biolink-block-id="<?= $data->link->biolink_block_id ?>" data-biolink-block-type="<?= $data->link->type ?>" class="col-12 col-lg-<?= ($data->link->settings->columns ?? 1) == 1 ? '12' : '6' ?> my-<?= $data->biolink->settings->block_spacing ?? '2' ?>">
    <a href="#" data-toggle="modal" data-target="<?= '#product_' . $data->link->biolink_block_id ?>" class="btn btn-block btn-primary link-btn <?= ($data->biolink->settings->hover_animation ?? 'smooth') != 'false' ? 'link-hover-animation-' . ($data->biolink->settings->hover_animation ?? 'smooth') : null ?> <?= 'link-btn-' . $data->link->settings->border_radius ?> <?= $data->link->design->link_class ?>" style="<?= $data->link->design->link_style ?>" data-text-color data-border-width data-border-radius data-border-style data-border-color data-border-shadow data-animation data-background-color data-text-alignment>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form id="<?= 'product_form_' . $data->link->biolink_block_id ?>" method="post" role="form" enctype="multipart/form-data">
                    ...
                    <?php if($data->link->settings->price): ?>
                        <div class="form-group">
                            <label for="<?= 'product_form_price_' . $data->link->biolink_block_id ?>"><?= l('biolink_product.price') ?></label>
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" name="email" id="<?= 'product_form_email_' . $data->link->biolink_block_id ?>" class="form-control form-control-lg" maxlength="320" placeholder="<?= l('global.email_placeholder') ?>" required="required" />
                        <small class="form-text text-muted"><?= l('biolink_product.email_help') ?></small>
                    </div>

                    <?php if($data->link->settings->price): ?>
                        <?php $has_plisio = false ?>
                        <?php $has_plisio_whitelabel = false ?>
                        <?php $has_offline_payment = false ?>

                        <div class="row form-group">
                            <?php
                            $payment_processors_count = count($data->link->settings->payment_processors_ids);
                            ?>
                            <?php foreach($data->link->settings->payment_processors_ids as $payment_processor_id): ?>
                                ...
                                <?php if($data->payment_processors[$payment_processor_id]->processor == 'offline_payment') $has_offline_payment = $payment_processor_id ?>

                                <label class="col-<?= $payment_processors_count <= 2 ? '12' : '6' ?> my-2 custom-radio-box">
                        ...
                        <?php if($has_offline_payment): ?>
                            <div class="form-group d-none" data-payment-processor-custom-container data-offline-payment-container>
                                <label><?= l('biolink_product.offline_payment.instructions') ?></label>
                                <div class="card"><div class="card-body"><?= $data->payment_processors[$has_offline_payment]->settings->instructions ?></div></div>
                            </div>

                            <div class="form-group d-none" data-payment-processor-custom-container data-offline-payment-container>
                                <div class="form-group">
                                    <label for="<?= 'product_form_proof_' . $data->link->biolink_block_id ?>"><?= l('biolink_product.offline_payment.proof') ?></label>
                                    <input type="file" name="offline_payment_proof" id="<?= 'product_form_proof_' . $data->link->biolink_block_id ?>" accept="<?= \Altum\Uploads::get_whitelisted_file_extensions_accept('payment_processors_offline_payment_proofs') ?>" class="form-control-file altum-file-input" required="required" />
                                    <small class="form-text text-muted"><?= sprintf(l('global.accessibility.whitelisted_file_extensions'), \Altum\Uploads::get_whitelisted_file_extensions_accept('payment_processors_offline_payment_proofs'))  . ' ' . sprintf(l('global.accessibility.file_size_limit'), settings()->links->offline_payment_proof_size_limit) ?></small>
                                </div>
                            </div>
                        <?php endif ?>
                    <?php endif ?>
                    ...
                    <div class="text-center mt-4">
                        <button type="submit" name="submit" class="btn btn-block btn-lg btn-primary" data-is-ajax><?= $data->link->settings->price ? l('biolink_product.submit') : l('biolink_product.free') ?></button>
                    </div>
                </form>
            </div>
...
<?php if(!\Altum\Event::exists_content_type_key('javascript', 'product')): ?>
    ...
    <script>
        ...
        document.querySelectorAll('<?= '#product_' . $data->link->biolink_block_id ?> [data-processor]').forEach(element => element.addEventListener('click', event => {
            let processor = event.currentTarget.dataset.processor;

            document.querySelectorAll('<?= '#product_' . $data->link->biolink_block_id ?> [data-payment-processor-custom-container]').forEach(element => element.classList.add('d-none'));

            if(processor === 'plisio') {
                document.querySelector('<?= '#product_' . $data->link->biolink_block_id ?> [data-plisio-container]').classList.remove('d-none');
            }
            else if(processor === 'plisio_whitelabel') {
                document.querySelector('<?= '#product_' . $data->link->biolink_block_id ?> [data-plisio-whitelabel-container]').classList.remove('d-none');
            }
            else if(processor === 'offline_payment') {
                document.querySelectorAll('<?= '#product_' . $data->link->biolink_block_id ?> [data-offline-payment-container]').forEach(offline_payment_element => offline_payment_element.classList.remove('d-none'));
            }
        }));

        /* Set default */
        document.querySelector('<?= '#product_' . $data->link->biolink_block_id ?> [name="payment_processor_id"]').click();
