...
class BiolinkBlockAjax extends Controller {
    ...
    private function order() {
        ...
        if(empty($_POST['biolink_blocks']) || !is_array($_POST['biolink_blocks']) || empty($_POST['link_id'])) {
            Response::json('', 'success');
        }
        ...
        foreach($_POST['biolink_blocks'] as $link) {
            ...
            $case_query .= ' WHEN ' . $link['biolink_block_id'] . ' THEN ' . $link['order'];
            $biolink_block_ids[] = $link['biolink_block_id'];
        }

        $in_query = implode(',', $biolink_block_ids);

        /* Update all blocks */
        db()->rawQuery("UPDATE `biolinks_blocks` SET `order` = CASE `biolink_block_id` {$case_query} END WHERE `user_id` = {$this->user->user_id} AND `biolink_block_id` IN ({$in_query})");
    ...
    private function update_biolink_weather() {
        ...
        /* Clear the cache */
        cache()->deleteItem('biolink_blocks?link_id=' . $biolink_block->link_id);
        cache()->deleteItem('weather_' .  $biolink_block->biolink_block_id);
    ...
    private function create_biolink_embeddable($type) {
        ...
        switch($type) {
			...
			case 'bandcamp':
				...
				$parsed_url = parse_url($_POST['location_url']);

				if(empty($parsed_url['scheme']) || empty($parsed_url['host']) || !in_array(strtolower($parsed_url['scheme']), ['http', 'https'], true)) {
					Response::json(l('link.error_message.invalid_location_url_embed'), 'error');
				}

				$host = strtolower(rtrim($parsed_url['host'], '.'));
				if($host !== 'bandcamp.com' && substr($host, -13) !== '.bandcamp.com') {
					Response::json(l('link.error_message.invalid_location_url_embed'), 'error');
				}
				...
				$response = \Unirest\Request::get($_POST['location_url'], [
					'User-Agent' => 'Mozilla/5.0 (compatible; BandcampEmbedFetcher/1.0)',
					'Accept' => 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8'
				]);

				if(empty($response->code) || $response->code < 200 || $response->code >= 300) {
					Response::json(l('global.error_message.basic'), 'error');
				}

				$html = is_string($response->body) ? $response->body : (string)($response->raw_body ?? '');

				if($html === '' || !preg_match('~<!--\s*(album|track)\s+id\s+(\d+)\s*-->~i', $html, $match)) {
					Response::json(l('link.error_message.invalid_location_url_embed'), 'error');
				}

				$settings['embed_type'] = strtolower($match[1]); /* album or track */
				$settings['embed'] = $match[2]; // id

				$settings['type'] = isset($_POST['type']) && in_array($_POST['type'], ['small', 'large']) ? query_clean($_POST['type']) : 'small';
				$settings['display_artwork'] = (int) isset($_POST['display_artwork']);
				$settings['display_track_list'] = (int) isset($_POST['display_track_list']);
