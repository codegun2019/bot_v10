...
class ApiLinks extends Controller {
	...
	private function post() {
		...
		$total_rows = db()->where('user_id', $this->user->user_id)->where('type', 'links')->getValue('links', 'count(`link_id`)');
		...
		if(empty($_POST['domain_id']) && !settings()->links->main_domain_is_enabled && $this->user->type != 1) {
	...
	private function patch() {
		...
		$total_rows = db()->where('user_id', $this->user->user_id)->where('type', $link->type)->getValue('links', 'count(`link_id`)');
	...
	private function check_url($url) {
		if($url) {
			...
			if(mb_strlen($url) < $this->user->plan_settings->url_minimum_characters ?? 1) {
				$this->response_error(sprintf(l('link.error_message.url_minimum_characters'), $this->user->plan_settings->url_minimum_characters ?? 1), 401);
			}

			if(mb_strlen($url) > $this->user->plan_settings->url_maximum_characters ?? 64) {
				$this->response_error(sprintf(l('link.error_message.url_maximum_characters'), $this->user->plan_settings->url_maximum_characters ?? 64), 401);
			}
		}
	}
