            ...
            <div class="collapse" data-parent="#biolink_theme_update" id="biolink_container">
                ...
                <div id="biolink_background_type_image">
                    <div class="form-group" data-file-image-input-wrapper data-file-input-wrapper-size-limit="<?= settings()->links->background_size_limit ?>" data-file-input-wrapper-size-limit-error="<?= sprintf(l('global.error_message.file_size_limit'), settings()->links->background_size_limit) ?>">
                        <label for="background_type_image_input"><?= l('link.settings.background_type_image') ?></label>
                        <?= include_view(THEME_PATH . 'views/partials/file_image_input.php', ['uploads_file_key' => 'biolink_background', 'file_key' => 'biolink_background_image', 'already_existing_image' => $data->biolink_theme->settings->biolink->background, 'display_preview' => !empty($data->biolink_theme->settings->biolink->background) && is_string($data->biolink_theme->settings->biolink->background) && !string_ends_with('.mp4', $data->biolink_theme->settings->biolink->background)]) ?>
                ...
                <div class="form-group">
                    ...
                    <div class="row btn-group-toggle m-n2" data-toggle="buttons">
                        <?php foreach(['scroll', 'fixed'] as $background_attachment): ?>
                            <div class="p-2 col-6">
                                <label class="btn btn-light btn-block font-size-small mb-0 text-truncate <?= $data->biolink_theme->settings->biolink->background_attachment == $background_attachment ? 'active"' : null?>">
                                    <input type="radio" name="biolink_background_attachment" value="<?= $background_attachment ?>" class="custom-control-input" <?= ($data->biolink_theme->settings->biolink->background_attachment ?? null) == $background_attachment ? 'checked="checked"' : null?> />
